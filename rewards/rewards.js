...
function makeList(argument) {
	...
	for (var i = 1 + argument * 10; i <= argument * 10 + 10; i++) {
		infList.innerHTML	=	infList.innerHTML
								...
								+	inf[i].img		+	'.png"></div><div class="box_floor_reward">'
								+	inf[i].reward	+	'</div><div class="box_floor_quan">'
								+	inf[i].quan		+	'</div></div>'
								;
	}
...
function infGoal() {
	...
	for (var i = Number(floor_start.value); i <= Number(floor_goal.value); i++) {
		if (itemCheck[inf[i].img] == 0) {
			...
			elem.innerHTML	=	'<div class="inf_result_box_img"><img title="'
								+	inf[i].reward	+	'" alt="\n'
								+	inf[i].reward 	+	' " src="item/'
